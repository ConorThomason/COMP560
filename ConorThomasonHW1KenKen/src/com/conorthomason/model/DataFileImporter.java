...
public class DataFileImporter {
    ...
    public void importData() throws FileNotFoundException {
        ...
        try {
            for (int i = 0; i < arraySize; i++) {
                ...
                for (int j = 0; j < arraySize; j++) {
                    ...
                    if (adjacentCellCheck(newCell.getCellKey(), i+1, j))
                        newCell.setRightCell(constrainedArray[i+1][j]);
                    if (adjacentCellCheck(newCell.getCellKey(), i - 1, j))
                        newCell.setLeftCell(constrainedArray[i-1][j]);
                    if (adjacentCellCheck(newCell.getCellKey(), i, j+1))
        ...
        Utils.printConstrainedArrayCharacters(constrainedArray);
        while (input.hasNext()){
            String currentLine = input.next();
    ...
    public boolean adjacentCellCheck(char key, int i, int j){
        try{
            if (constrainedArray[i][j].getCellValue() == key)
                return true;
            else
                return false;
            ...
            //Again, normal. No guarantee the cells have been initialized yet.
            return false;
        }
    }
